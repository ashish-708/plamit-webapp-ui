...
function getActiveChildRoute(pathname: string, children: { route: string }[] = []) {
  return children
    .filter((child) => matchesRoute(pathname, child.route))
...
export function AppSidebar({
  ...
  return (
    <aside
      className={cn(
        "hidden h-dvh shrink-0 border-r border-border bg-white text-sidebar-foreground shadow-[8px_0_28px_rgba(39,37,54,0.04)] transition-all lg:top-0 lg:z-50 lg:flex lg:flex-col",
        position === "fixed" ? "lg:fixed lg:left-0" : "lg:sticky",
            ...
            <div className="space-y-1">
              {visibleItems
                ...
                .map((item) => {
                  ...
                  const moduleRoot = `/${item.route.split("/").filter(Boolean)[0] ?? ""}`;
                  const active = matchesRoute(pathname, item.route) || Boolean(activeChildRoute) || (hasChildren && matchesRoute(pathname, moduleRoot));
                    ...
                    return (
                      <div key={item.id}>
                        <button
                          ...
                          onClick={() => {
                            if (collapsed) {
                              ...
                              setOpenItems((current) => ({ ...current, [item.id]: true }));
                              return;
                            }
                        ...
                        >
                          ...
                          {!collapsed ? <ChevronDown className={cn("h-4 w-4 shrink-0 transition", expanded && "rotate-180")} /> : null}
                        </button>
                        {!collapsed && expanded ? (
                          <div className="ml-4 mt-1 space-y-1 border-l border-border pl-2">
                            {item.children?.map((child) => {
                              const childIsActive = child.route === activeChildRoute;
                              return (
                                <Link
                                  className={cn(
